...
function scheduleFollowUps(senderId) {
  ...
  const intervals = [
    { time: 60*60*1000, msg: "Hi ভাইয়া/আপু! 😊 আপনার ওয়েডিং কার্ডের অর্ডারটি কি কনফার্ম করব? আমাদের স্টক সীমিত, তাই দ্রুত জানালে ভালো হয়! 🛍️" },
    { time: 2*60*60*1000, msg: "কোনো ডিজাইন কি পছন্দ হয়েছে আপু/ভাইয়া? 💖 কোনো কাস্টমাইজেশন লাগলে আমাদের জানাতে পারেন কিন্তু! ✨" },
    { time: 6*60*60*1000, msg: "শুভ দিন! 🌸 বিয়ের কার্ডের অর্ডারটি আজই কনফার্ম করলে দ্রুত ডেলিভারি পেয়ে যেতেন। আপনার সিদ্ধান্তের অপেক্ষায় রইলাম! 🥰" },
    { time: 24*60*60*1000, msg: "আসসালামু আলাইকুম ভাইয়া/আপু। 💌 BOONDHON-এর সাথে যোগাযোগ করার জন্য আপনাকে অনেক ধন্যবাদ। কোনো হেল্প লাগলে জানাবেন, ভালো থাকবেন! 🙏❤️" }
  ];
...
function detectTrigger(text) {
  ...
  if (["🙅 না, আর লাগবে না","stop_images"].some(w => t.includes(w))) return "stop_images";
  if (["👍 হ্যাঁ, বাকিগুলো দাও","continue_images"].some(w => t.includes(w))) return "continue_images";
  ...
  if (["english","ইংরেজি","🇬🇧"].some(w => t.includes(w))) return "english_form";
  return null;
}
...
async function getGeminiReply(senderId, userMessage) {
  ...
  const systemText =
    ...
    "━━━━━ বিজনেস গাইডলাইন ━━━━━\n" +
    "১. টোন: অত্যন্ত মিষ্টি, আন্তরিক ও সেলস ক্লোজিংমুখী। প্রচুর সুন্দর ইমোজি (🌸, ✨, 💰, 🛍️, 🥰) ব্যবহার করবে।\n" +
    "২. প্রাইস রুলস:\n" +
    ...
    "   - অফার: কেবল ২০০ পিস বা তার বেশি নিলে FREE নিকাহনামা।\n" +
    "৩. দাম আপত্তি: প্রথমবার quality বলো, কমাবে না। ৩ বার আপত্তিতে ৫ টাকা ডিসকাউন্ট।\n" +
    "৪. ২০ পিস সিক্রেট: নিজে বলবে না। জোর করলে বলবে: ২০ পিস = ১,৫০০ টাকা।\n" +
    "৫. Customer যাই বলুক বুঝে reply দাও। button ছাড়াও কথা বলতে পারবে।\n" +
    "৬. কখনো AI বলবে না। সবসময় বাংলায় কথা বলবে।\n" +
    "৭. হটলাইন: 01701016826 (WhatsApp ও একই)।";
  ...
  for (const entry of body.entry) {
    ...
    if (trigger) {
      ...
      if (trigger === 'stop_images') {
        await sendMainMenu(senderId, "ঠিক আছে ভাইয়া/আপু! 🥰 অন্য কিছু লাগলে জানাবেন। অর্ডার ফাইনাল করব? 🛍️");
      ...
      if (trigger === 'affordable') {
        ...
        await sendMsg(senderId, "🌸 Affordable কালেকশনের ডিজাইনগুলো পাঠানো হচ্ছে... একটু অপেক্ষা করুন ভাইয়া/আপু! 👇");
      ...
      if (trigger === 'premium') {
        ...
        state.lastImageIndex = 0;
        await sendMsg(senderId, "✨ Premium কালেকশনের এক্সক্লুসিভ ডিজাইনগুলো পাঠানো হচ্ছে... 👇");
        await sendImagesStream(senderId, 0);
        continue;
      }
      ...
      if (trigger === 'price') {
        ...
        await sendMainMenu(senderId,
          ...
          '🎁 ধামাকা অফার: ২০০ পিস বা বেশি নিলে FREE নিকাহনামা! 🥰\n\n' +
      ...
      if (trigger === 'delivery') {
        ...
        await sendMainMenu(senderId,
          '🚚 ডেলিভারি ও পলিসি:\n\n' +
          '📍 অফিস: মানিকগঞ্জ\n' +
          '🏭 কারখানা: ফকিরাপুল, লালবাগকেল্লা, বাংলাবাজার ও বাবুবাজার\n\n' +
          '📦 অর্ডারের নিয়ম:\n' +
          ...
          '🗺️ Map: https://maps.app.goo.gl/CnyRST5KxHjWDAtd9'
        );
        scheduleFollowUps(senderId);
        continue;
      }
      ...
      if (trigger === 'price_objection') {
        ...
        if (state.priceObjectionCount < 3 && !state.hasOfferedDiscount) {
          ...
          await sendMainMenu(senderId,
            '🌸 ভাইয়া/আপু, শুধুমাত্র আপনার জন্য ম্যানেজমেন্ট থেকে স্পেশাল পারমিশন নিলাম! 🥰\n\n' +
            'প্রতি পিসে ৫ টাকা স্পেশাল ডিসকাউন্ট দেওয়া যাবে! 🥳\n\n' +
            'এই ওয়ান-টাইম অফারে এখনই অর্ডার করবেন? 🛍️'
          );
        } else {
          await sendMainMenu(senderId, "আহারে ভাইয়া/আপু! 😭 এর চেয়ে কম করলে আমাদের লস হবে। যোগাযোগের জন্য ধন্যবাদ! 🙏❤️");
        }
        scheduleFollowUps(senderId);
        continue;
      }
