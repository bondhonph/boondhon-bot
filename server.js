...
// ড্রাইভ ইমেজ আইডি সমূহ (Affordable & Premium)
...
// ইমেজ ইউআরএল জেনারেটর (ফিক্সড স্ট্রিলিং ইন্টারপোলেশন)
...
function detectTrigger(text) {
  ...
  if (["দাম","প্রাইস","price","কত টাকা","কত","অফার","দাম ও অফার"].some(w => t.includes(w))) return "price";
  if (["ডেলিভারি","কারখানা","delivery","অফিস","ঠিকানা","পলিসি","পলিসি ও ঠিকানা"].some(w => t.includes(w))) return "delivery";
...
async function sendMainMenu(recipientId, mainText) {
  try {
    if (mainText) { await sendMsg(recipientId, mainText); }
    await axios.post(`https://graph.facebook.com/v19.0/me/messages?access_token=${PAGE_TOKEN}`, {
      recipient: { id: recipientId }, message: { attachment: { type: "template", payload: { template_type: "button", text: "👇 নিচের অপশনগুলো সিলেক্ট করুন:", buttons: [
    ...
    await axios.post(`https://graph.facebook.com/v19.0/me/messages?access_token=${PAGE_TOKEN}`, {
      recipient: { id: recipientId }, message: { attachment: { type: "template", payload: { template_type: "button", text: "⚙️ অন্যান্য তথ্য:", buttons: [
...
async function sendImagesStream(senderId, startIndex = 0) {
  ...
  await sendMainMenu(senderId, "✨ সব ডিজাইন দেখানো শেষ! 🥰 কোনটি পছন্দ হয়েছে? অর্ডার কনফার্ম করব? 🛍️");
}
...
async function getGeminiReply(senderId, userMessage) {
  ...
  try {
    const response = await axios.post(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${GEMINI_API_KEY}`, {
      contents, systemInstruction: { parts: [{ text: systemText }] }, generationConfig: { maxOutputTokens: 250, temperature: 0.7 }
    });
    return response.data?.candidates?.[0]?.content?.parts?.[0]?.text || 'জি ভাইয়া/আপু! 🥰 কীভাবে সাহায্য করতে পারি?';
  } catch (err) {
  ...
  for (const entry of body.entry) {
    ...
    if (trigger) {
      ...
      if (trigger === 'stop_images') {
        await sendMainMenu(senderId, "ঠিক আছে ভাইয়া/আপু! 🥰 অর্ডার ফাইনাল করব? 🛍️");
    ...
    // 🚨 টেলিগ্রাম নোটিফিকেশন অ্যালার্ট সিস্টেম (ফর্ম পূরণ করলে এডমিনের কাছে মেসেজ যাবে)
    if (userText.includes('নামঃ') || userText.includes('Name:')) {
      ...
      if (telegramBotToken && telegramChatId) {
        try {
          const alertMessage = `🚨 *নতুন ওয়েডিং কার্ড অর্ডার এসেছে!* 🚨\n\n👤 *কাস্টমার আইডি:* ${senderId}\n📋 *ফর্মের ধরণ:* ${userText.includes('নামঃ') ? 'বাংলা ফর্ম' : 'English Form'}\n\n📝 *ডিটেইলস:* \n${userText}`;
          
          await axios.post(`https://api.telegram.org/bot${telegramBotToken}/sendMessage`, {
            chat_id: telegramChatId, text: alertMessage, parse_mode: 'Markdown'
          });
        } catch (err) { console.error('Telegram Error:', err.message); }
      }
      await sendMsg(senderId, "🎉 অনেক ধন্যবাদ ভাইয়া/আপু! আপনার ইনফো ফর্মটি আমাদের সিস্টেমে সফলভাবে জমা হয়েছে। আমাদের টিম দ্রুত আপনার সাথে ডেমো ডিজাইন নিয়ে যোগাযোগ করবে। 🌸");
      scheduleFollowUps(senderId);
      continue;
    }
...
app.listen(PORT, () => console.log('🚀 BOONDHON Bot Engine Live and Stable!'));
